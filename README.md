...
│ (画像)   │ (五层)   │ (多源适配) │   (跨源混排)               │
├──────────┴──────────┴───────────┴────────────────────────────┤
│  LLM 适配层  ·  B 站 API  ·  XHS Sidecar (GPL隔离)  · SQLite │
└─────────────────────────────────────────────────────────────┘
```
...
B 站走 API 直连（WBI 签名），小红书走 GPL 隔离的 sidecar 容器 + 扩展代理发现（不自动滚动、不后端爬取），通用 Web 走 Playwright CDP + LLM 内容提取。
...
│   │   ├── xiaohongshu_adapter # 小红书 (HTTP sidecar)
│   │   └── web_adapter        # 通用 Web (Playwright + LLM)
│   ├── bilibili/              # B 站接入层 (WBI 签名 · 速率控制)
│   ├── llm/                   # 多模型 LLM 适配
│   └── storage/               # 数据存储层
├── sidecar/xhs-downloader/    # 小红书 sidecar (GPL-3.0 隔离容器)
...
| 小红书交互 | GPL 隔离 sidecar 容器 (XHS-Downloader) + 扩展代理发现 |
| 存储 | SQLite + Embedding 向量索引 |
| 容器化 | Docker Compose (后端 + xhs-sidecar) |
| Agent 框架 | 自研轻量框架 |
