...
- 与 fork 改动无真冲突：`popup.js` / `popup.html` / `app.py` / web 与 desktop CSS / desktop `app.js` 均因改动落在不同区域而自动合并，仅 `changelog.md` 需手动合并。合并后全 `.py` 编译 + repost / marketing / soul / api 关键测试通过。
